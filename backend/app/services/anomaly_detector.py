"""
PulseDebug AI — Anomaly Detector
==================================
File: backend/app/services/anomaly_detector.py
Purpose:
    Deterministic rule-based anomaly detection.
    Fixed: replaced conn.execute() with database.execute() helper
    so UPDATE queries work with both SQLite and PostgreSQL.
...
from app.core.database import execute, commit
...
class AnomalyDetector:
    ...
    def evaluate(
        ...
        *,
        ...
        if stats.has_enough_history and stats.error_rate >= settings.ERROR_RATE_THRESHOLD:
            is_anomaly = True
        ...
        # Write anomaly flag back using the database helper
        if is_anomaly:
            execute(conn,
                "UPDATE api_logs SET is_anomaly = 1 WHERE id = ?",
                (log_id,),
            )
            commit(conn)
