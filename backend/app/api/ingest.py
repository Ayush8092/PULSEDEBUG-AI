...
Purpose:
    ...
    Updated to use database.execute(), fetchone(), lastrowid() helpers
    so ingestion works with both SQLite and PostgreSQL.
...
from app.core.database import (
    get_connection, execute, fetchone, fetchall,
    commit, lastrowid, USING_POSTGRES,
)
...
def _confidence_from_label(label: str) -> float:
    return {"High Likelihood": 0.85, "Moderate Likelihood": 0.60,
            "Needs Investigation": 0.35}.get(label, 0.50)


async def _run_rca_background(incident_id: int) -> None:
    conn = get_connection()
    try:
        row = fetchone(
            execute(conn, "SELECT * FROM incidents WHERE id = ?", (incident_id,))
        )
        if not row or row.get("ai_summary"):
            return
        ...
        stats_row = fetchone(
            execute(conn,
                ...
                """,
                (row["service"], row["endpoint"], row["first_detected"], row["last_seen"]),
            )
        )

        logs_summary = {}
        if stats_row and stats_row["total"]:
            logs_summary = {
                "total_events":   stats_row["total"],
                "anomaly_count":  stats_row["anomalies"],
                "avg_latency_ms": round(stats_row["avg_lat"] or 0, 1),
                "max_latency_ms": stats_row["max_lat"] or 0,
                "server_errors":  stats_row["srv_err"],
                "client_errors":  stats_row["cli_err"],
                "error_rate":     round(
                    (stats_row["srv_err"] + stats_row["cli_err"]) / stats_row["total"], 3
                ),
            }
...
async def ingest_event(
    ...
    try:
        ...
        if USING_POSTGRES:
            # PostgreSQL needs RETURNING id because lastrowid is not supported
            cur = execute(conn,
                """
        ...
        if is_anomaly:
            ...
            if incident_id:
                add_timeline_event(
                    ...
                    detail=f"{payload.error_signature or 'Unknown'} on {payload.endpoint}",
                )
                background_tasks.add_task(run_correlation_engine, incident_id)

                if ai_status()["available"]:
                    inc_row = fetchone(
                        execute(conn, "SELECT severity FROM incidents WHERE id=?", (incident_id,))
                    )
                    if inc_row and inc_row["severity"] in ("critical", "warning"):
                        background_tasks.add_task(_run_rca_background, incident_id)
                        ai_queued = True
